...
public interface ReparacionRepository extends JpaRepository<Reparacion, Long> {
    ...
    @Query(value= "select count(*) \n" +
            ...
            "and id_estado = 4 \n" +
            "and id_taller = ?3) or (fecha_devolucion = DATE(?1) and hora_devolucion > ?2 and id_taller = ?3 and id_estado = 4) or (fecha_reserva = DATE(?1) and id_taller = ?3 and id_estado = 1)", nativeQuery = true)
